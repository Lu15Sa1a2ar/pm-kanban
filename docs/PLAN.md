...
- [x] Put the test count and the timing numbers in one exported constant so they are updated in one place. They came from the Part 14 and Part 15 runs and will drift. `lib/facts.ts` (name, links, 121 automated tests = 50 pytest + 53 Vitest + 13 mocked Playwright + 5 integrated Playwright, 290 ms, 1.9 s, 10 messages).
