...
- [ ] Full suites pass locally against SQLite, then on Docker, then on production after the deploy. Local and Docker done on 2026-09-18: backend 48/48, Vitest 53/53, mocked Playwright 12/12, integrated 4/4 on Docker (SQLite). A card the copilot creates without notes now shows an italic "Double-click to add notes" placeholder so the details stay editable, and the system prompt asks the model to draft one sentence of details (in the language of the question) when the user gives none; 4/4 real requests in English and Spanish came back with fitting details. After the user's first local pass: busy states with a spinner on `Try the demo`, `Sign in` and `Log out`, a `Loading your board` screen while the session and the board load, and a pressed state (`active:scale-[0.98]`) on the primary buttons. Production pending the deploy.
