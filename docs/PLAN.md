...
- [x] Playwright against production (`app.spec.ts` headers test): the CSP contains `object-src 'none'` and `frame-src 'none'`, and `Permissions-Policy` is present. Passed on 2026-09-19 against `6aa06cd`; `/api/ai/connectivity` and `/docs` return 404 on production.
