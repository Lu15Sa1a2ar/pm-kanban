...
- `tests/integrated/assistant.spec.ts` exercises the real assistant through the browser (6 AI messages, own 6-minute timeout). OpenRouter requests pin `provider.require_parameters` and ignore DeepInfra because it returns plain text for JSON-schema requests; a non-JSON completion is still shown as a text-only reply.
